...
To achieve these goals, we're building a resource with the following objectives:
...
* Users of AusTraits are encouraged to recognise contributions (via citation) and also contribute towards the further improvement of the database.
...
## Structure of raw data files for AusTraits {#structure2}
...
## Building & rebuilding AusTraits {#building2}
...
Limited taxonomic information is provided in the main table `data`, only `original_name` (the original name provided by the authors) and `species_name` (the aligned taxonomic name; see [species taxonomy](#taxonomy) for details). But further information is available in the table `taxonomy`:
...
traits_spread$value %>%
  mutate(specific_leaf_area = as.numeric(specific_leaf_area), 
         photosynthetic_rate_per_area = as.numeric(photosynthetic_rate_per_dry_mass_saturated)) %>%
  ggplot(aes(x=specific_leaf_area, y = photosynthetic_rate_per_dry_mass_saturated, colour =site_name)) +
  geom_point()
```
...
traits <- austraits$traits %>% 
  filter(dataset_id == "GrassBase_2014")
traits
```
...
```
```
## Error: Each row of output must be identified by a unique combination of keys. Keys are shared for 6452 rows .....
```
