...
To assist you in constructing the `metadata.yml` file, we have developed functions to help fill in the different sections of the file. You cqn then manually edit the file further to fill in missing details.  
...
The skeletal `metadata.yml` file created by the function `metadata_create_template` includes a template for entering details about people. Edit this manually, duplicating if details for multiple people are required.
...
Add a single date in a `yyyy-mm` format to an entire dataset
```{r, eval=FALSE, echo=TRUE}
...
- *value_type*: See the bottom of `system.file("support", "austraits.build_schema.yml", package = "austraits.build")` for a list of accepted value types.  
...
2. Then, in Excel, add a column named `description` and write out a description for each factorial context, such as `Plants were growth at high light (1800 PAR), low CO2 (300 ppm), and were well-watered`. Save as a .csv file.  
...
Toward the top of the metadata is a section titled `dataset:`. This needs to be filled in manually.  
