...
This sections explains how to format files to contribute a new study to AusTraits. Before starting this, you should read more about 

- the [structure of the compiled AusTraits database](http://traitecoevo.github.io/austraits.build/articles/austraits_structure.html), 
- the [structure of the raw data files](http://traitecoevo.github.io/austraits.build/articles/austraits_raw_data.html), and 
...
After you've added the custom R code to a file, check that it has completed the intended dataframe manipulation: 
...
The tests also identify mis-matches between the site names in the data.csv file vs. metadata.yml file (same for context), unsupported trait names, etc. 
...
- *Missing value*: Value was missing. Do nothing – these almost always represent honestly missing data. Large numbers of missing values are often present if two dataframes with values for different traits are connected together with `bind_rows()` or if duplicate values have been omitted using custom R code.  
...
2. Taxon alignments made by `metadata_check_taxa` are added to the `taxonomic_updates:` section of the `metadata.yml` file
    ...
    If the species is obviously a mis-spelled Australian species, but was mismatched, manually correct the correct alignment.
...
For a study requiring a large number of manual taxonomic changes, you can create a dataframe with headers `dataset_id`, `find` (original name), `replace` (APC/APNI accepted name), and `reason` to manually fill in and read into the `taxonomic_updates` section:
...
data %>%
  ...
  ungroup()
```
Npte: Using custom R code instead of filtering the values in the data.csv file itself, ensures the relevant trait values are still associated with each line of data in the data.csv file, but only read into AusTraits a single time. 
Note: You would use `group_by(Species, Site)` if there are unique values at the species x site level.
