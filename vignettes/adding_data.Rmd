...
6. Add the new study into the build framework and rebuild AusTraits.
...
source("scripts/taxonomy.R")          # source functions required to use our taxonomy matching workflow 
...
1. Columns within the `data.csv` file must include `taxon name`, `site_name` (if appropriate), `contexts` (if appropriate), and `collection_date` (if appropriate). The `data.csv` file can either be in a wide format (1 column for each trait, with `trait name` as the column header) or long format (a single column for all `trait values` and additional columns for `trait name` and `units`) 

  a.   For all field studies, ensure there is a column for `site name`. If all measurements were made at a single site, a site column can easily be mutated using [custom_R_code](#custom_R) within the metadata.yml file. See sections [adding sites](#adding_sites) and [adding contexts](#adding_contexts) below for more information on compiling site and context data.
  ...
  c. [Creating context names]{#creating_context_name}: there must be a single column of context names. If there is a single `context_property` such as `growing season` or `leaf position`, the context names are general the various values for the context_property, such as `wet_season` and `dry_season` or `sun_leaves` and `shade_leaves`. If a study records two or more context properties, a context name column must be generated that is effectively unique factorial combinations of the various context properties. For instance, in an experimental study, measurements may have been made under `high` vs. `low` light, `high` vs. `low` CO2 concentration, and `well-watered` vs. `drought` conditions. A contributor will likely have three columns, labeled, `light_levels`, `CO2_concentration` and `water_treatment`, which need to be merged into a single column, `context`. The contributor's three context columns can be merged to create unique values for each factorial treatment as follows. The protocol for adding contextual properties to the metadata file is under [adding contexts](#adding_contexts)   
...
- You can identify runs of columns by column number/position. For instance `c(5:25), .fns = mean` or `c(` leaf area`:`leaf N`), .fns = mean`
...
- **population_id** Population_id must be specified if the unique combinations of site and context do not properly specify the groupings that should be assigned to the `pop` component of `entity_id`. For instance, in some cases the `context_property` is actually repeat measurements (wet vs dry season); in these cases population_id should be based only on the site name and needs to be assigned.
  ...
  **WARNING** The same column name cannot be assigned to two fields within `metadata$dataset`. Since the column header with site names is already assigned to `metadata$dataset$site_name`, it can not be reused under population_id. Instead you have to mutate an identical column in custom R code to assign to population_id. For instance ```data %>% mutate(population_id = site_name)```

- **individual_id** Individual_id is one of the fields that can be read in during `metadata_create_template`. However, you may instead mutate your own `individual_id` using `custom_R_code` and add it in manually. For a wide dataset individual_id is required anytime there are multiple rows of data for the same individual and you want to keep these linked. As with `population_id` this field should only be included if it is required. 
...
- **collection_type:**  Allowable values include: field, botanical collection, field_experiment, glasshouse, literature. This field can also be read in from a column or can be specified at the site or trait level, as described below. Entries under `metadata$sites` or `metadata$traits` (which apply to only that specific site or trait) override the global value entered under `metadata$dataset`.

- **sample_age_class:**   Allowable values include: adult, sapling, seedling, juvenile. This field can also be read in from a column or can be specified at the site or trait level, as described below. Entries under `metadata$sites` or `metadata$traits` (which apply to only that specific site or trait) override the global value entered under `metadata$dataset`.
...
- **measurement_remarks**: Measurement remarks is a field to indicate either (1) notes that accompany method numbers; or (2) a notes or references column within the spreadsheet that is explicitly linked to a specific trait (or all traits) and therefore is not captured by "methods" (which is fixed to a single value for a trait). It can be read in for the whole dataset, or entered under [dataset$traits](#add_traits) 

- **observation_number** If the same entity is measured repeatedly across time, the repeat measurements need to be assigned sequential observation numbers. These numbers will usually be generated through [custom R code](#custom_R), and the field `observation_number` will be added under either `dataset$metadata` or [dataset$traits](#add_traits) as appropriate.

- **method_number** If different columns (wide format) or rows (usually long format) of data represent measurements made on the same trait using different methods, the different methods need to be assigned sequential method numbers. These numbers will usually be generated through [custom R code](#custom_R), and the field `method_number` will be added under either `dataset$metadata` or [dataset$traits](#add_traits) as appropriate. If you use method_number, also include some brief notes under `measurement_remarks` to indicate what differs between the methods. (e.g. `method_number: 1`, matches with `measurement_remarks: fully expanded leaves` versus `method_number: 2` matches with `measurement_remarks: leaves still expanding`). 

- **entity_id** As background, `population_id` and `individual_id` are used during the build process to create an `entity_id`. Also required is `entity_type` which is usually input in the [traits](#add_traits) section. Overall, `entity_id` is built from up to three segments, a taxon_id segment (for all entity types), a population_id segment (when `entity_type` is `population` or `individual`), and a individual_id segment (when `entity_type` is `individual`).
...
In addition to the automatically propogated fields, there are a number of optional fields you can add if appropriate.

- **sample_age_class** If all measurements in a dataset were made on plants of the same `sample age class` (life stage) a global value should be entered under [metadata$dataset](#metadata_dataset). However if different traits were measured at different life stages or different rows of data represent measurements at different life stages you can specify a unique sample age class for each trait or indicate a column where this information is stored.

- **collection_type** If all measurements in a dataset represent the same `collection type` a global value should be entered under [metadata$dataset](#metadata_dataset). However if different traits have different collection types or different rows of data represent different collection types you can specify a unique collection type for each trait or indicate a column where this information is stored.
...
5. Although site properties are not restricted to a controlled vocabulary, newly added studies should use the same site property syntax as others whenever possible, to allow future discoverability. To generate a list of already used under `site_property`, use: 
...
1. Under [creating context names](#creating_context_name) above was the example of an experimental study where measurements were made under `high` vs. `low` light, `high` vs. `low` CO2 concentration, and `well-watered` vs. `drought` conditions. To create a single column with context name, we merged the contributor's three columns, `light_levels`, `CO2_concentration` and `water_treatment` into a single column, `context` creating unique values for each factorial treatment.  

2. As with site data, extract a list of unique contextual property combinations from the data.csv file

```{r, eval=FALSE, echo=TRUE}
read_csv("data/dataset_id/data.csv") %>%
  distinct(context, .keep_all = TRUE) %>% # the argument `.keep_all` ensures columns aren't dropped
  select(context, light, CO2, water) %>% # list of relevant columns to keep
  rename(`water treatment` = water, `light treatment` = light, `CO2 treatment` = CO2 %>% 
      # rename columns to how you want them to appear in the metadata file. Faster to do it once here than repeatedly in the metadata file
  write_csv("data/dataset_id/raw/context_data.csv")
```

3. Although contextual properties are not restricted to a controlled vocabulary, newly added studies should use the same contextual property syntax as others whenever possible, to allow future discoverability. To generate a list of terms already used under `context_property`, use: 
...
You are first prompted to identify the column with the context name and then to list all columns that contain context properties. This automatically fills in the context component on the metadata file.  
...
2. Taxon alignments made by `metadata_check_taxa` are added under `metadata$taxonomic_updates` 
