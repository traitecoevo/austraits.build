...
- alternatively, use functions we've created explicitly for pre-processing data, found in [pre_process.R](https://github.com/traitecoevo/austraits.build/blob/master/R/pre_process.R) and loaded through `library(austraits.build)`
...
dataset_test_setup(dataset_ids)

# Tests run test on all studies
dataset_ids <- dir("data")
dataset_test_setup(dataset_ids)
```
Fix as many errors as you can and then rerun `dataset_test_setup()` repeatedly until no errors remain. 
...
austraits_rebuild_taxon_list()
austraits <- remake::make("austraits")
dataset_generate_report(current_study)
```
...
austraits <- remake::make("austraits")
dataset_generate_report(current_study, overwrite = TRUE)
```

To generate a report for a collection of studies:  
```{r, eval=FALSE, echo=TRUE}
dataset_generate_reports(c("Falster_2005_1", "Wright_2002"), overwrite = TRUE)
```

Or for all studies:  
```{r, eval=FALSE, echo=TRUE}
dataset_generate_reports(overwrite = TRUE)
```
