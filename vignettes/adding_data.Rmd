...
```{r, eval=FALSE, echo=TRUE}
metadata_check_taxa(dataset_id, max_distance_abs = 3, max_distance_rel = 0.2, try_outside_guesses = FALSE)
```
...
data %>% 
  group_by(Species) %>%
  mutate_at(vars(leaf_percentN, `plant growth form`),
...
3. For the above example, then add the following code to custom R code, that removes the duplicate values from the data columns (by seeting them as `NA`) as the dataset is read into AusTraits.
