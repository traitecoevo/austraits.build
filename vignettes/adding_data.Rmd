...
austraits <- read_rds("export/austraits.rds")
...
1. Clone the `austraits.build` repository from github and create a new branch in the repo with the name `dataset_id`, e.g. `Gallagher_2014`. 
2. Create a new folder within the folder `data` with the name `dataset_id`, e.g. `Gallagher_2014`.
...
Our preferred format for `dataset_id` is the surname of the first author of any corresponding publication, followed by the year, as `surname_year`. E.g. `Falster_2005`. Wherever there are multiple studies with the same id, we add a suffix `_2`, `_3` etc. E.g.`Falster_2005`, `Falster_2005_2`.

## Constructing the `data.csv` file {#csv_file} 

All data for a study (`dataset_id`) must be merged into a single spreadsheet: `data.csv` or read in through the `metadata.yml` file. Throughout this section, there will be references to information that can be input through the `data.csv` file or the `metadata.yml` file.

1. Columns must include `taxon name`, `site_name` (if appropriate), `contexts` (if appropriate), and `sample_date` (if appropriate). Trait data can either be in a wide format (1 column for each trait, with `trait name` as the column header) or long format (1 column for all `trait values` and additional columns for `trait name` and `units`) See sections [adding sites](#adding_sites) and [adding contexts](#adding_contexts) below for more information on compiling site and context data.

  a.   For all field studies, ensure there is a column for `site name`. If all measurements were made at a single site, a site column can easily be mutated using [custom_R_code](#custom_R) within the metadata.yml file.

  b.   If available, be sure to include a column with `collection date`. If possible, provide \dates in `yyyy-mm-dd` (e.g. 2020-03-05) format or, if the day of the month isn't known, as `yyyy-mm` (e.g. 2020-03). However, any format is allowed and the column can be parsed to the  proper yyyy-mm-dd format using custom_R_code. As with sites, if the same `collection date` applies to the entire study it can be added through [custom_R_code](#custom_R) or directly into the metadata.yml file.
  
  c.   [Creating context names]{#creating_context_name}   
    
  Variation in contextual values must be summarised in a single column. This is easy in most circumstances, where context values will be a single column of values, distinguishing, for instance, between `wet_season` vs. `dry_season` or `sun_leaves` vs. `shade_leaves`. It can become much more difficult for experimental studies with multiple manipulations applied factorially to the study plants. For instance, measurements may have been made under `high` vs. `low` light, `high` vs. `low` CO2 concentration, and `well-watered` vs. `drought` conditions. A contributor will likely have three columns, labeled, `light_levels`, `CO2_concentration` and `water_treatment`, which need to be merged into a single column, `context`. The contributor's three context columns can be merged to create unique values for each factorial treatment as follows. The protocol for adding contextual properties to the metadata file is under [adding contexts](#adding_contexts)   
...
2. Data submitted by a contributor should be in the rawest form possible; always request data with individual measurements over site/species means. Some studies make replicate measurements on an individual. For these studies, individual means need to be calculated. `austraits$traits` does not include multiple measurements per individual, although the data is preserved in the contributor's raw data files. Be sure to calculate the number of replicates that contributed to each mean value.
...
read_csv("data/dataset_id/raw/raw_data.csv") %>%
  ...
  group_by(individual, `species name`, site, context, etc) %>%
  summarise_at(
    vars(leaf_area, `leaf N`),.funs = mean,
    vars(replicates), .funs = sum,
    vars(growth_form, `photosynthetic pathway`), .funs = first) %>%
  ungroup()
```
...
- You can identify runs of columns by number. For instance `var(5:25), .funs = mean`
...
original_trait, new_trait, original_values, values_for_new_trait, values_to_keep
```{r, eval=FALSE, echo=TRUE}
data %>% move_values_to_new_trait(original_trait= "growth form", new_trait = "parasitic", original_values = "parasitic", values_to_keep = "parasitic")
```
 
 or
 
```{r, eval=FALSE, echo=TRUE}
data %>% move_values_to_new_trait("fruits", "dispersal_appendage", c("dry & winged", "enclosed in aril"), 
                              c("wings", "aril"), c(NA, "enclosed")))
```
...
9. You can do manipulations, such as adding a column with `sites` or manipulating `site names`. This is only recommended for studies with a single (or few) site, where manually adding the site data to the `metadata.yml` file is fast, since in precludes automatically propagating site data into metadata (see [Adding site details](#Adding site details)). As an example (Blackman_2010 dataset): 
...
11. You can generate `method_numbers` and corresponding `measurement_remarks` if rows of data were collected using different methods

```{r, eval=FALSE, echo=TRUE}
data %>%
  mutate(
        method_number = ifelse(material == "FRESH",1,NA),
        method_number = ifelse(material == "DRIED",2,method_number),        
        method_number = ifelse(material == "SENESCED",3,method_number),
...
- **population_id** Population_id must be specified if the unique combinations of site and context do not properly specify the groupings that should be assigned to the `pop` component of `entity_id`. For instance, in some cases the `context_property` is actually repeat measurements (wet vs dry season); in these cases population_id should just be based on the site name and needs to be assigned.
  
  **WARNING** population_id must be omitted if it isn't used; `population_id: unknown` will overwrite all the actually population information
  
  **NOTE** Since the site_name column is already assigned to site_name, it can not be reused under population_id. Instead you have to mutate an identical column in custom R code to assign to population_id. For instance ```data %>% mutate(population_id = site_name)```

- **individual_id** Individual_id is one of the fields that can be read in during `metadata_create_template`. However, you may instead mutate your own `individual_id` using `custom_R_code` and add it in manually. For a wide dataset need an individual_id anytime there are multiple rows of data for the same individual and you want to keep these linked. As with `population_id` this field should only be included if it is required. 

- **collection_date** If this is not read in as a specified column, it needs to be filled in manually as `start date/end date` in yyyy-dd-mm format. If the collection dates are unknown, write `unknown/publication year`
...
- **collection_type:**  Possible values include: field, botanical collection, field_experiment, glasshouse, literature. This field can also be read in from a column or can be specified at the site or trait level, as described below. `site` and `trait` level entries override dataset-level entries.

- **sample_age_class:**   Possible values include: adult, sapling, seedling, juvenile. This field can also be read in from a column or can be specified at the site or trait level, as described below. `site` and `trait` level entries override dataset-level entries.
...
- **measurement_remarks**: Measurement remarks is a field to indicate either (1) notes that accompany method numbers; or (2) a notes or references column within the spreadsheet that is explicitly linked to a specific trait (or all traits) and therefore is not captured by "methods" (which is fixed to a single value for a trait). It can be read in for the whole dataset, or applied to [specific traits](#add_traits) 

- **observation_number** If the same entity is measured repeatedly across time, the repeat measurements need to be assigned sequential observation numbers. These numbers will usually be generated through [custom R code](#custom_R) and a column read in through `dataset$metadata`.

- **method_number** If different rows of data represent measurements made using different methods, the different methods need to be assigned sequential method numbers. These numbers will usually be generated through [custom R code](#custom_R) and a column read in through `dataset$metadata`. If you use method_number, also include some brief notes under `measurement_remarks` to indicate what differs between the methods. (e.g. `measurement_remarks: fully expanded leaves`; `measurement_remarks: leaves still expanding`). Measurement_remarks read in at the dataset level are best created through custom R code or there may be a column with the relevant information in the data.csv file.

As background, `population_id` and `individual_id` are used during the build process to create an `entity_id`. Also required is `entity_type` which is usually input in the [traits](#add_traits) section. Overall, `entity_id` is built from up to three segments, a taxon_id segment (for all entity types), a population_id segment (when `entity_type` is `population` or `individual`), and a individual_id segment (when `entity_type` is `individual`).
...
- **units**: fill in the units specified by the author - such as mm2. If you're uncertain about the syntax/format used for some more complex units, look through the schema file (`system.file("support", "austraits.build_schema.yml", package = "austraits.build")`) or the file showing unit conversions (`system.file("support", "austraits.build_schema.yml", package = "austraits.build")`). For categorical variables, leave this as `.na`.  
...
- **entity_type**: Entity types indicate the "level" to which trait values refer. Entity types can be individual, population, species, genus, family or order. It is an essential component to later compiling `entity_id`

- **value_type**: Value types can be mean, minimum, maximum, mode, range, raw, and bin. See the top of `system.file("support", "austraits.build_schema.yml", package = "austraits.build")` for definitions of these accepted value types.  

- **basis_of_value**: Basis of value indicates how a value was determined. Allowable terms are measurement, expert_score, model_derived, and literature. See the top of `system.file("support", "austraits.build_schema.yml", package = "austraits.build")` for definitions of these accepted value types.  
...
In general, methods sections extracted from pdfs include "special characters" (non-UTF-8 characters). Non-English alphabet characters are recognised (e.g. é, ö) and should remain unchanged. Other characters will be re-formatted during the study input process, so double check that degree symbols (º), en-dashes (–), em-dashes (—), and curly quotes (‘,’,“,”) have been maintained or reformatted with a suitable alternative. Greek letters and some other characters are replaced with their Unicode equivalent (e.g. <U+03A8> replaces Psi (Ψ)); for these it is best to replace the symbol with an interpretable English-character equivalent.
  ...
  ```
    methods: &leaf_length_method All measurements were from dry herbarium collections, with leaf and bracteole
    measurements taken from the largest of these structures on each specimen.
  ```
    Then for the next trait that uses this method you can just include. For paragraph-long methods sections this keeps the metadata file much shorter.
  ```
    methods: *leaf_length_method
  ```

In addition to the automatically propogated fields, there are a number of fields you can add if appropriate.

- **sample_age_class** If all measurements in a dataset were made on plants of the same `sample age class` (life stage) this information can be added under [metadata$dataset](#metadata_dataset). However if different traits were measured at different life stages or different rows of data represent measurements at different life stages you can specify a unique sample age class for each trait or indicate a column where this information is stored.

- **collection_type** If all measurements in a dataset represent the same `collection type` this information can be added under [metadata$dataset](#metadata_dataset). However if different traits have different collection types or different rows of data represent different collection types you can specify a unique collection type for each trait or indicate a column where this information is stored.

- **measurement_remarks**: Measurement remarks is a field to indicate either (1) notes that accompany method numbers; or (2) a notes or references column within the spreadsheet that is explicitly linked to a specific trait (or all traits) and therefore is not captured by "methods" (which is fixed to a single value for a trait)

- **observation_number** Occasionally repeat measurements on the same trait might be presented in separate columns in the `data.csv` file. In this case, for the trait in the column that captures the first measurement, add `observation_number: 1` and for the trait in the column that captures the second measurement, add `observation_number: 2`

- **method_number** If different columns in a wide data.csv file indicate measurements on the same trait using different methods, they need to be sequentially assigned, `method_number: 1`; `method_number: 2`, etc. If you use method_number, also include some brief notes under `measurement_remarks` to indicate what differs between the methods. (e.g. `measurement_remarks: fully expanded leaves`; `measurement_remarks: leaves still expanding`)
...
5. Although site properties are not restricted to a controlled vocabulary, it is best to use the same site property syntax as others, so future discoverability. To generate a list of already used site_properties, use:

```
austraits$sites %>% distinct(site_property)
```
...
It is possible that you will want to specify **sample_age_class** or **collection_type** at the site_level. You can later add these fields to some or all sites.


### Context details {#adding_contexts}  

Context properties are formatted much like site properties, where the context name (generated above) is simply a container for a unique combination of contextual property values.

As with site information, this data needs to be compiled into a table (or manually added into metadata for simple studies). 
...
2. As with site data, extract a list of unique contextual property combinations from the data.csv file

```{r, eval=FALSE, echo=TRUE}
read_csv("data/dataset_id/data.csv") %>%
  distinct(contxt, .keep_all = TRUE) %>% # the argument `.keep_all` ensures columns aren't dropped
  select(context, light, CO2, water) %>% # list of relevant columns to keep
  rename(`water treatment` = water, `light treatment` = light, `CO2 treatment` = CO2 %>% # rename columns to how you want them to appear in the metadata file. Faster to do it once here than repeatedly in the metadata file
  write_csv("data/dataset_id/raw/context_data.csv")
```

3. Although contextual properties are not restricted to a controlled vocabulary, it is best to use the same context property syntax as others, so future discoverability. To generate a list of already used context_properties, use:

```
austraits$contexts %>% distinct(context_property)
```

4. As with site data, you could open it in Excel at this point to make manual edits.

5. This context data can then be read into `metadata.yml`:  
```{r, eval=FALSE, echo=TRUE}
...
There will be some follow-up editing in the metadata$context section.

1. Add `[manipulated]` to contextual properties that represent experimental manipulations.

2. Ensure that the value of each contextual property is clear. 

```{r, eval=FALSE, echo=TRUE}
  grown_at_Elv_temp_and_280_ppm_CO2_and_harvested_at_80_days:
    age at harvest (days): 80
    CO2 treatment: 280 ppm (sub-ambient) [manipulated]
    temperature treatment: elevated (30/22 deg C (day/night)) [manipulated]
    measurement temperature (C): .na.character
  grown_at_Elv_temp_and_280_ppm_CO2_and_measurements_made_at_28_deg_C_and_harvested_at_150_days:
    age at harvest (days): 150
    CO2 treatment: 280 ppm (sub-ambient) [manipulated]
    temperature treatment: elevated (30/22 deg C (day/night)) [manipulated]
    measurement temperature (C): ambient (28 deg C; current growing season temperature)
      [manipulated]
```
...
Each row of data should have a unique combination of the following fields:
  taxon_name, site_name, context_name, entity_id, observation_number, method_number, value_type, original_name

Therefore, the dataset should be able to pivot wider and the following code should have a `1` in every cell. (Note that as of June 2022, a small bug in the taxon matching algorithm means that trait values for many orchids are being duplicated in AusTraits and this isn't working for the entire dataset combined, but does work if you make a single dataset.)

```{r, eval=FALSE, echo=TRUE}
austraits$traits %>%
  select(trait_name, value, taxon_name, site_name, context_name, entity_id, observation_number, method_number, value_type, original_name) %>%
  pivot_wider(names_from = trait_name, values_from = value, values_fn = length) %>% View()
```

## Running tests {#running_tests}

Begin by running some automated tests to ensure the dataset meets the required set up. The tests run through a collection of pre-specified checks on the files for each study. The output alerts you to possible issues needing to be fixed, by comparing the data in the files with the expected structure and allowed values, as specified in the schema and trait_definitions.   
...
test_data_setup(dataset_ids)

# Tests run test on one study using `current_study` variable
dataset_ids <- current_study
test_data_setup(dataset_ids = current_study)

# Tests run test on all studies
dataset_ids <- dir("data")
test_data_setup()
```
Fix as many errors as you can and then rerun `test_data_setup()` repeatedly until no errors remain. 
...
2. Taxon alignments made by `metadata_check_taxa` are added to the `taxonomic_updates:` section of the `metadata.yml` file
    ...
    a. On occasion there are nonsensical realignments due to fuzzy matching. These most often occur when measurements were made on non-naturalised horticultural plants, whose names do not appear on the APNI/APC lists. If this is the case, add these taxa to the excluded_data list. For instance, in eFLOWER_2021, the inclusion of many non-native species in the submitted dataset led to the following two incorrect substitutions. These need to be manually deleted, with the species added to the excluded data section.
    ...
    If the species is obviously a misspelled Australian species, but was mismatched, manually correct the alignment.
...
```
or by manually typing the information into the `metadata.yml` file:
```{r, eval=FALSE, echo=TRUE}
- find: Senna sturtii
...
A final quality check is conducted by generating a report on the data in each study. 

Rebuild the taxon list and AusTraits to ensure other changes made during the quality checks were implemented. Then build the study report:  

```{r, eval=FALSE, echo=TRUE}
austraits_rebuild_taxon_list()
austraits <- remake::make("austraits")
build_dataset_report(current_study)
```
...
austraits <- remake::make("austraits")
build_dataset_report(current_study, overwrite = TRUE)
```

To generate a report for a collection of studies:  
```{r, eval=FALSE, echo=TRUE}
build_dataset_reports(c("Falster_2005_1", "Wright_2002"), overwrite = TRUE)
```

Or for all studies:  
```{r, eval=FALSE, echo=TRUE}
build_dataset_reports(overwrite = TRUE)
```
