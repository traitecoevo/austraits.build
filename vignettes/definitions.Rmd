...
definitions <- austraits:::austraits_lite_post$definitions
```


This document provides a full list of trait definitions used in AusTraits version `r desc::desc_get_field("Version")`, as defined in the file configuration files [`config/traits.yml`](https://github.com/traitecoevo/austraits.build/blob/master/config/traits.yml).

```{r, results="asis"}
for (trait in names(definitions)) {
