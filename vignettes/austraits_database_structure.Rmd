...
- `entity_id` is built from up to three segments, a taxon_id segment (for all entity types), a population_id segment (when `entity_type` is `population` or `individual`), and a individual_id segment (when `entity_type` is `individual`).
    ...
    - The population_id segments numbers an alphabetised list of locations or, in some cases, location by context combinations, sequentially, producing segments `pop01`, `pop02`, etc. For experimental and manipulative studies, "population" is defined as a group of individuals subjected to the same treatment conditions. 
