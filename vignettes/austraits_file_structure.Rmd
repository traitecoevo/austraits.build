...
├── inst/schema           # Defines the overall austraits schema
...
The file `traits.yml` provides the trait definitions used to compile AusTraits, allowable trait values, and definitions of all elements included in the tables that comprise AusTraits. The [trait definitions](http://traitecoevo.github.io/austraits.build/articles/Trait_definitions.html) is fully described in additional vignettes. A `.yml` file is a structured data file where information is presented in a hierarchical format (see [appendix for details](#yaml)). 
...
- If the data is in wide format, each row should include measurements made on a single individual or a single species-by-site mean, with different trait values as consecutive columns. 
...
The metadata is compiled in a `.yml` file, a structured data file where information is presented in a hierarchical format (see [Appendix for details](#yaml)).  There are `r length(austraits$schema$metadata$elements)` values at the top hierarchical level: `r sprintf("%s", austraits$schema$metadata$elements %>% names()) %>% paste(collapse = ", ")`. These are each described below.
...
This section provides `r tolower(austraits$schema$metadata$elements$source$description)` In general we aim to reference the primary source. References are written in structured yml format, under the category `source` and then under sub-groupings `primary` and `secondary`. General guidelines for describing a source include:
...
This section provides `r tolower(austraits$schema$metadata$elements$contributors$description)`. The following information is recorded for each data contributor:

```{r, echo=FALSE, results="show"}
austraits$schema$metadata$elements$contributors$elements$data_collectors %>%
  list1_to_df() %>%
...
??Daniel why won't the descriptions show up in the next 2 lines?

- Assistants, `r tolower(austraits$schema$metadata$elements$assistants$description)`
 
- AusTraits dataset curators, `r tolower(austraits$schema$metadata$elements$austraits_curators$description)`

#### Dataset

This section includes `r tolower(austraits$schema$metadata$elements$dataset$description)`

The following elements are included under the element `dataset`:


??Daniel how do I filter out values I don't want in this table (observation_id). And trait_name, value are odd, because they are indicating the column for these traits in long datasets, but the definitions in the table below are for the output in the traits table??

```{r}
values <- austraits$schema$metadata$elements$dataset$values
...
The fields `collection_date`, `replicates`, `basis_of_value`, `value_type`, `sample_age_class`, `collection_type`, `measurement_remarks`, `observation_number`, and `method_number` can all be specified at the dataset level or the traits level (which overrides a dataset-level entry) or sites level (which also overrides a dataset-level entry). In each case, they can be a fixed text value or indicate a column within the data.csv file (or generated through `custom_R_code`) that includes the relevant information. 

- `sample_age_class`, `collection_type`, and `collection_date` are usually included under `metadata$dataset` unless they vary by trait.
...
A common use of the `custom_R_code` is to automate the conversion of a verbal description of flowering or fruiting periods into the supported trait values, as occurs in  this example. It might also be used if values for a single trait are expressed across multiple columns and need to be merged. See `Catford_2014` as an example of this. The [adding data](http://traitecoevo.github.io/austraits.build/articles/adding_data.html) vignette provides additional examples of code regularly implemented in `custom_R_code`, including functions specifically developed for AusTraits data manipulations. 

#### Traits

This section provides `r stringr::str_replace(austraits$schema$metadata$elements$traits$description,"A","a")`

For each trait submitted to AusTraits, there is the following information:

```{r}
values <- austraits$schema$metadata$elements$traits$elements
for (value in names(values)) {
  sprintf("- **%s**: %s", value, values[[value]]) %>% writeLines()
}
```

Values under `trait_name` must be allowable values, list in the `trait definitions` [file](https://github.com/traitecoevo/austraits.build/blob/master/config/traits.yml) or [vignette](http://traitecoevo.github.io/austraits.build/articles/Trait_definitions.html). Similarly, values under `value_type` must be allowable values, also listed in the definitions file.

The fields `replicates`, `basis_of_value`, `value_type`, `sample_age_class`, `collection_type`, `measurement_remarks`, `observation_number`, and `method_number` can all be specified at the dataset level or the traits level (which overrides a dataset-level entry). In each case, they can be a fixed text value or indicate a column within the data.csv file (or generated through `custom_R_code`) that includes the relevant information. 
...
This section provides `r tolower(austraits$schema$metadata$elements$substitutions$description)`
...
```{r}
values <- austraits$schema$metadata$elements$substitutions$values
for (value in names(values)) {
  sprintf("- **%s**: %s", value, values[[value]]) %>% writeLines()
}
```
...
This section provides `r tolower(austraits$schema$metadata$elements$taxonomic_updates$description)`

Each substitution is documented using the following elements:

```{r}
values <- austraits$schema$metadata$elements$taxonomic_updates$values
for (value in names(values)) {
  sprintf("- **%s**: %s", value, values[[value]]) %>% writeLines()
}
```
...
This section provides `r tolower(austraits$schema$metadata$elements$questions)`
