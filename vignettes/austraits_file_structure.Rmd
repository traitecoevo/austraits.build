...
The file `traits.yml` provides the trait definitions used to compile AusTraits, allowable trait values, and definitions of all elements included in the tables that comprise AusTraits. The [trait definitions](http://traitecoevo.github.io/austraits.build/articles/definitions.html) is fully described in additional vignettes. A `.yml` file is a structured data file where information is presented in a hierarchical format (see [appendix for details](#yaml)). 
...
Values under `trait_name` must be allowable values, list in the `trait definitions` [file](https://github.com/traitecoevo/austraits.build/blob/master/config/traits.yml) or [vignette](http://traitecoevo.github.io/austraits.build/articles/definitions.html). Similarly, values under `value_type` must be allowable values, also listed in the definitions file.
