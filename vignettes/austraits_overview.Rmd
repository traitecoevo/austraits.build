...
* For **categorical traits** the `definitions` file includes a list of allowable `values`, allowed terms for the trait. Each categorical trait value is defined in the `definitions` file. Lists of substitutions translate the exact syntax and terms in the spreadsheet submitted into the values allowed by AusTraits. This ensures that for a certain trait the same `value` has an identical meaning throughout the AusTraits database.
