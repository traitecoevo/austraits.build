...
Each trait measurement has an associated `value_type`, which gives ``r austraits$definitions$value_types$description``. Possible values are:

```{r value_types}
austraits$definitions$value_types$values %>% 
  list1_to_df()  %>% 
  my_kable_styling() %>% 
  writeLines()
```
...
A list of taxonomic name changes that have been made, documented in the following format:
