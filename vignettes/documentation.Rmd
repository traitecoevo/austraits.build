...
It is essential that future users of AusTraits data are confident the data have the meaning they expect it to and were collected using methods they trust. As such, each dataset within Austraits must incclude descriptions of the study, sites, and methods used as well as the data itself.
...
Below is the standard definition for each trait in AusTraits (drawn from the the file `config/definitions.yml`). Traits are labelled as either `numeric` or `categorical`.
...
The folder `data` conatins a long list of folders, one for each study and each conatining two files:
...
where `Angevin_2010`, `Barlow_1981`, `Bean_1997`, `Blackman_2014` are each a unique `dataset_id` in the final dataset.
...
Within the compiled matrix `austraits` are the elements listed in [Elements of Austraits](#elements). The actual trait data can be found in the dataframe `austraits$data`.
...
The data.csv file can be in either long or wide format.
...
The metadata is compiled in a `.yml` file, a structured data file where information is presented in a hierarchical format (see [Appendix for details](#yaml)).  There are 7 values at the top hierarchical level: `source`, `people`, `dataset`, `config`, `traits`, `substitutions`, `taxonomic_updates` and `questions`. These are each described below.

As a start, checkout some examples from [existing studies in Austraits](https://github.com/traitecoevo/austraits/tree/master/data), e.g. [Angevin_2010](https://github.com/traitecoevo/austraits/blob/master/data/Angevin_2010/metadata.yml) or [Wright_2004](https://github.com/traitecoevo/austraits/blob/master/data/Wright_2004/metadata.yml).

XXXXnext section doesn't make sense here - seems like these are examples, not part of compiling the metadata fileXXXXX
...
unlink("tmp.yml")
```
XXXXXend section to moveXXXXX


A blank `metadata.yml` file is stored in XXXX and can be duplicated as a starting point.

We have developed a number of functions to help fill in the different sections of the metadata.yml file. These are indicated in their respective sections below.
...
Two functions are available to facilitate entering source data.
...
`add_doi("key","doi")`
```
 and the different elements within source will automatically be generated.

If your data is unpublished (or otherwise without a `doi`) use the function `add_source` with the following syntax: 

```{r, eval=FALSE, echo=TRUE}
`add_source("key", "bibtype","author","year","title")`
```

If you wish to include additional elements, you can afterwards add them manually to the `metadata.yml` file.

### People

This includes a list of the different contributors to the study, their respective institutions and roles in the study.

Data can be entered manually or using the function XXXXX `add_people()` with the required arguments 
```{r, eval=FALSE, echo=TRUE}
`add_people("name","institution","role")`
```
...
If a person has multiple roles, such as being both the `lab leader` and the person to `contact` with questions about the data set, use the following syntax when entering the data: 

```{r, eval=FALSE, echo=TRUE}
`add_people("name","institution",c("role1","role2"))`
```

Note that only the AusTraits custodians have the contributors e-mails on file. This information will not be directly available to AusTraits users or new contributors via Github.

### Dataset

The following data are included under the element `Dataset`:

**year_collected_start:** `r sprintf(austraits$definitions$austraits$elements$details$elements$year_collected_start)`

**year_collected_end:** `r austraits$definitions$austraits$elements$details$elements$year_collected_end`

**description:** `r austraits$definitions$austraits$elements$details$elements$description`

**collection_type:** `r austraits$definitions$austraits$elements$details$elements$collection_type`

**sample_age_class:** `r austraits$definitions$austraits$elements$details$elements$sample_age_class`

**sampling_strategy:** `r austraits$definitions$austraits$elements$details$elements$sampling_strategy`
...
The function `add_dataset` propagates the elements within `dataset` using the following syntax:

```{r, eval=FALSE, echo=TRUE}
`add_dataset("year_start","year_end","collection_type","age_class","original_file")`
```

This function will also create the elements `description`, `sampling_strategy`, and `notes`, but you must fill those manually in the `metadata.yml` file. `description` and `sampling_strategy` will be temporarily filled with `unknown` and `notes` will be filled with `.na`.

### Config

The data under `config` indicates the basic format of the submitted data file. It includes three components, `is_vertical`, `variable_match`, and `custom_R_code`.

**is_vertical** is the field to identify your data as being in a long versus wide format. For a long format `is_vertical` should be `yes` and for a wide format `is_vertical` should be `no`.

**variable_match** `r austraits$definitions$metadata$elements$config$elements$variable_match` The relevant column header is the value, as in, `species name: column header with species names`. Ignore other columns in `data.csv`for now.

**custom_R_code** is where you enter any R code that is required to convert the `data.csv` file into a format that can be compiled by AusTraits. A common use is to automate the conversion of a verbal description of flowering or fruiting periods into the supported trait values. It might also be used if values for a single trait are expressed across multiple columns and need to be merged. See the `metadata.yml` file in the folder `Catford_2014` as an example of this.

See [additional examples in the appendix on adding Custom R code](#custom_R_code)

### Traits

The level `traits` provides two distinct functions. The first is to map the contributed traits onto the Austraits trait names and units. The second is to document information on the collection methods and meaning of the submitted trait values. In particular for each trait submitted to Austraits, there is the following information:

**var_in** This is the trait name used as a column header in a wide `data.csv` file or the trait name given under the `trait` column in a long `data.csv` file.

**unit_in** These are the units used in the submitted `data.csv` file. These are converted to standardized units for that trait through the file `unit_conversions.csv` in the folder `config`. If no units are given an educated guess is made based on the values submitted.

**trait_name** The corresponding trait name used in `austraits`. See the section [trait definitions](#trait_defs) for a list of the possible trait names for AusTraits.

**value_type** Each trait measurement has an associated `value_type`, which gives ``r austraits$definitions$value_type$description`` Possible values are listed under [Data structures > Value types](#value_types)

**replicates** The number of replicate measurements that were made for values identified as type  `mean`, `min`, or `max`. This element should be assigned `.na` for value_types `expert_mean`, `expert_min` and `expert_max`. If the number of replicates is `unknown`, enter `unknown`.

**methods** The methods used to collect the given trait measurements. Whenever possible, this information is lifted near verbatim from a published source.

### Substitutions

The `metadata.yml` file for each study also contains a list of trait value substitutions that are applied, as needed, to categorical variables in the dataset. These changes are documented in the following format:
...
For instance, substitutions are required if the exact word(s) used to describe a categorical trait value in AusTraits are different from the vocabulary used by the author in the `data.csv` file. It is preferable that authors make changes using `substitutions` rather than changing the `data.csv` file. See the section [trait definitions](#trait_defs) for a list of supported values for each trait.

Substitutions can be added by running:

XXXXX need to create function

```{r, eval=FALSE, echo=TRUE}
`add_substitution("dataset_id","trait_name", "find", "replace")`
```
...
There are additional arguments you can add for the function `check_study_taxa` including:
...
Therefore, if you want the function  `check_study_taxa` to correct 1 and 2 character typos, run the function as follows:

```{r, eval=FALSE, echo=TRUE}
check_study_taxa("Blackman_2014",typos=TRUE)
```
...
Next, you run the following code to compile just your new dataset into the AusTraits format. 

XXXXX when I added datasets I realized it was easier to run lines 1 at a time to identify faster where there were errors.
...
austraits$data
```
(Note that data appears as a `tibble` rather than a `data.frame`. Tibbles are an enhanced type dataframe, with some desirable features, such as not printing the whole dataframe to screen. (If you prefer, you can easily use a traditional R `data.frame` by converting it using `df <- data.frame(austraits$data)`. But from here on we'll assume you're sticking with a tibble.)
...
Limited taxonomic information is provided in the main table `data`, only `original_name` (the original name provided by the authors) and `species_name` (the aligned taxonomic name; see [species taxonomy](#species_taxonomy) for details). But further information is available in the table `species_list`:
...
As the entire austraits dataset is quite large, we recommend first sub-setting the data before spreading from long to wide format. You may also need to reduce the number of measurements in any one observation (see next section). For this first example we'll sue the dataset `Blackman_2014`. 
