...
**Reporting Errors**: If you notice a possible error in Austraits, please [post an issue here](https://github.com/traitecoevo/austraits/issues), describing the error and possible fix in detail. If you can, please provide code illustrating the problem.
...
- You collected data on one of the traits list in the [trait definitions table](config/definitions_traits.csv).
...
It is essential that future users of AusTraits data are confident the data have the meaning they expect it to and were collected using methods they trust. As much, each dataset within Austraits must incorporate descriptions of the study, sites, and methods used with the data itself.
...
Below is the standard definition for each trait in AusTraits (drawn from the the file `config/definitions.yml`). Traits are labelled as either `numeric` or `categorical`. Numeric traits also include 
...
The contents of the folder `data` then have a structure as follows:
...
XXXXnext section doesn't make sense hereXXXXX
...
unlink("tmp.yml")
```
XXXXXend section to move

A blank `metadata.yml` file is stored in XXXX and can be duplicated as a starting point.

We have developed a number of helper functions to fill in the data for the different sections of this file. These are indicated in each of the sections below.
...
Data can be entered manually or using the function XXXXX `add_people()` with the required arguments `add_people("name","institution","role")`
...
XXXX`add_people("name","institution",c("role1","role2"))`
...
XXXXfunction - not sure a function makes as much sense here - or maybe a function to input year start, year end, collection type, age class
...
**variable_match** `r austraits$definitions$metadata$elements$config$elements$variable_match` The syntax is, for instance, `species_name: column in data.csv with species name`. Ignore other columns in `data.csv`for now.
...
See [example in the appendix on adding Custom R code](#custom_R_code)

XXXXnot sure functions useful here

### Traits

The level `traits` provides two distinct functions. The first is to map the contributed traits into the Austraits trait names and units. The second is to document information on the collection and meaning for the submitted trait values. In particular for each trait submitted to Austraits, there is the following information:
...
**value_type** Each trait measurement has an associated `value_type`, which gives ``r austraits$definitions$value_type$description`` Possible values are list under [Data structures > Value types](#value_types)

**replicates** The number of replicates measurements that were made for values identified as a type of `mean`, `min`, or `max`. This element should be assigned `.na` for value_types `expert_mean`, `expert_min` and `expert_max`.
...
add_substitution(trait_name, find, replace)
```
Where `find` is the trait_value used in the data.csv file and `replace` is the trait value supported by Austraits.
