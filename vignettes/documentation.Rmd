...
Data from each study is organised into a separate folder, with two files:
  - `data.csv`: a table containing the actual trait data a table of data in comma-separated values format, ideally with traits as columns and each observation on a single row.
...
The contents of the folder data then have structure like follows:
...
where `Angevin_2010`, `Barlow_1981`, `Bean_1997`, `Blackman_2014` are all a unique `dataset_id` in the final dataset.
...
2. Prepare the files `data.csv` an `metadata.yml` and place within the folder, as per instructions below. 
3. Add the new study into build framework and rebuild Austraits
4. Run tests on the contributed data and generate a report on the data, to make sure everything is formatted optimally. Return to step 3 if changes are made
5. Push to GitHub. 
...
About data.csv... Can be long or wide. Required columns .....


May require custom R code to get into right format

Keep data and file in rawest form possible (i.e. as few changes as possible)   but must be a single csv file. Other files used to create this (e.g. Excel ...) should be supplied so they can be archived. (XXXX Where to put these)
...
We have developed a number of helper functions to create the file `metadata.yml`. As a start,
