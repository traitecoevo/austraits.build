...
It is essential that future users of AusTraits data are confident the data have the meaning they expect it to and were collected using methods they trust. As such, each dataset within Austraits must include descriptions of the study, sites, and methods used as well as the data itself.
...
```{r, eval=FALSE}
create_metadata_template(dataset_id)
```
Asks a series of questions

Add custom R code and check it works
```{r, eval=FALSE}
...
```{r, eval=FALSE}
dataset_id <- "Falster_2005_1"

austraits$sites %>% filter(dataset_id == dataset_id) %>% select(-dataset_id) %>% spread(site_property, value) %>% type_convert() -> site_data
metadata_add_sites(dataset_id, site_data)
```
...
metadata_add_source_doi(dataset_id, type, doi, key=dataset_id)
```
...
```{r, eval=FALSE, echo=TRUE}
metadata_add_source(dataset_id, type, source = list(key= dataset_id, bibtype ="Misc", author = "Falster, DS", year = "2010",title = "Traits of Ku-ringai plant species"))
```
...
The data under `config` indicates the basic format of the submitted data file. It includes three components, `data_is_long_format`, `variable_match`, and `custom_R_code`.

**data_is_long_format** is the field to identify your data as being in a long versus wide format. For a long format `data_is_long_format` should be `yes` and for a wide format `data_is_long_format` should be `no`.
...
```{r, eval=FALSE, echo=TRUE}
source("R/setup.R")
metadata_check_taxa("Blackman_2014")
```
...
```{r, eval=FALSE, echo=TRUE}
metadata_check_taxa("Blackman_2014",typos=TRUE)
```
...
austraits$sources %>% bib_print() %>% sprintf("- %s", .) %>% writeLines()
```
