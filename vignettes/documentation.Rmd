...
Within the matrix `austraits` are the elements listed in [Elements of Austraits](#elements). The actual trait data is located in the dataframe `austraits$data`.
...
Data from each study is organised into a separate folder, with two files:<br/>
  - `data.csv`: a table containing the actual trait data a table of data in comma-separated values format, ideally with each trait in a single column and each observation on a single row (i.e. wide format).<br/>
  - `metadata.yml`: contains further information about the study, mapping of trait names and units onto standard types, and any substitutions applied to the data in processing (see below). 
...
Below are the Instructions on the formatting files for each study and contributing into Austraits. It is important that all steps are followed so that our automated workflow proceeds without problems.
...
3. Prepare the files `data.csv` and `metadata.yml` and place them within the folder, as per instructions below. 
4. Add the new study into the build framework and rebuild Austraits (see [Adding data to Austraits] (#adding) below)
5. Run tests on the contributed data and generate and proof a report on the data. This ensures everything is formatted optimally. In particular, check that numeric trait values fall within a logical range relative to other studies and that individual trait observations are not unnecessarily excluded because their trait values are unsupported. 
6. Return to step 3 if changes are made to the `data.csv` or `metadata.yml` files.
7. Push to GitHub. 
...
If a person has multiple roles, such as being both the `lab leader` and the person to `contact` with questions about the data set, using the following syntax when entering the data: 
...
**trait_name** The corresponding trait name used in `austraits`. See 
...
The `metadata.yml` file for each study also contains a list of substitutions that are applied to specific variables in the dataset. These changes are documented in the following format:
...
Once you have prepared your data files within a folder in the `data` directory, you can incorporate the new data into Austraits by running:
...
XXXXnot yet in perfect spot, but following code was in an odd location
```{r, eval=FALSE}
