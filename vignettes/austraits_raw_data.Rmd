---
title: Structure of raw data for building AusTraits
date: "`r Sys.Date()`"
...
The file `definitions.yml` defines the structure of the database and all terms. The tables and definitions provided in the section on Structure of AusTraits are defined in this file, as are trait definitions. A `.yml` file is a structured data file where information is presented in a hierarchical format (see [appendix for details](#yaml)). You will need to edit this file if you are changing or adding trait definitions.
...
read_csv("config/species_list.csv") %>%
  select(species_name, family, authority, TPL_ID, status, APC_name, APC_ID, APNI_ID) %>%
  slice(1:10) %>%
  my_kable_styling()
```
...
austraits$definitions$metadata$elements$people$elements$role$values %>%
  list1_to_df() %>%
  my_kable_styling()
```
...
Values under `trait_name` must be allowable values, as described under [the section trait definitions. Similarly, values under `value_type` must be allowable values, as described under the section  Value types.
...
This section provides `r tolower(definitions$metadata$elements$taxonomic_updates$description)` Taxonomic updates are required to align species names with an accepted species list.
