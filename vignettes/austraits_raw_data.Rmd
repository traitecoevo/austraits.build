...
The file `definitions.yml` defines the structure of the database and all terms. The tables and definitions described in the vignette [Structure of AusTraits data compilation](http://traitecoevo.github.io/austraits.build/articles/austraits_structure.html)  are defined in this file, as are trait definitions. A `.yml` file is a structured data file where information is presented in a hierarchical format (see [appendix for details](#yaml)).

The file `taxon_list.csv` is our master list of known taxa. Each species is listed once, with links to species' identifiers provided by the Australian Plant Name Index (APNI). The file `taxon_list.csv` is added to if a study includes taxa not previously represented in AusTraits. These can be names either on the APC/APNI lists, a compilation of taxonomic concepts for plants that are either native to or naturalised in Australia, or taxa without recognised names.  
...
This section includes `r tolower(definitions$metadata$elements$dataset$description)`
...
This section includes `r tolower(definitions$metadata$elements$config$description)`
...
This section provides `r stringr::str_replace(definitions$metadata$elements$traits$description,"A","a")`
...
Values under `trait_name` must be allowable values, list in the `definitions` [file](https://github.com/traitecoevo/austraits.build/blob/master/config/definitions.yml) or [vignette](http://traitecoevo.github.io/austraits.build/articles/Trait_definitions.html). Similarly, values under `value_type` must be allowable values, also listed in the definitions file.
...
This section provides `r tolower(definitions$metadata$elements$taxonomic_updates$description)`
