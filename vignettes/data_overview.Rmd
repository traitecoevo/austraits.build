...
There are __`r nrow(austraits$data)`__ trait records contained within Austraits database which belong to __`r austraits$data$species_name %>% unique() %>% length()`__ species and __`r austraits$species_list$family %>% unique() %>% length()`__ plant families.
...
There are __`r austraits$data$trait_name %>% unique() %>% length()`__ traits listed on the database.
...
data_geo <- 
  left_join(by=c("dataset_id", "site_name"),
      select(austraits$data, -original_name), sites) %>%
...
for(trait in traits_numeric[33:35]) {
  ...
  trait_distribution_plot_numerical(austraits, trait, "dataset_id", hide_ids = TRUE)
...
```{r categorical, results='asis', echo=FALSE, eval=F}
