...
species <- data_study$traits %>%
  ...
  left_join(select(austraits$taxonomy, species_name, TPL_ID), by ="species_name")  %>%
  mutate(link = ifelse(!is.na(TPL_ID),
           sprintf("[%s](%s)%s", species_name,  as_url_tpl(TPL_ID), 
                        ifelse(original_name!=species_name, sprintf(" (originally %s)", original_name), "")),
           species_name)
  ) %>%
  distinct() %>%
...
We have records on the following species from your study. We have attempted to align species names with [`The Plant List` (TPL)](http://www.theplantlist.org/) -- a global working list of all known plant species. Species recorded in your study are listed below, with a link given for those appearing in TPL:


`r species$link %>% paste(collapse=", ")`.
