...
Data from each study in Austraits is organised is organised into two files, within its own folder:
...
Data from those files has been used to generate the reports below.
...
- ` `r dataset_id`/data.csv` and ` `r dataset_id`/metadata.yml`: raw inputs for your study.
...
2. Edited versions of the input files for you study: ` `r dataset_id`/data.csv` and ` `r dataset_id`/metadata.yml` (if appropriate).

# Study context

We collect information about the context of any traits measurements. This includes details on the people involved, the sources, and details about type of collection, including location and time of collections. 
...
- **collectors**: The person(people) leading data collection (up to 2 permitted)
- **lab_leader**: The person leading data collection
- **contributor**: The person responsible for entering data into Austraits 
...
for(trait in traits_numeric) {
  ...
  methods <- metadata_study$traits %>% filter(trait_name == trait)
  ...
  c("", "",
    ...
    sprintf("- **Value type**: %s", methods$value_type),
    sprintf("- **Replicates**: %s", methods$replicates),
    sprintf("- **Methods (from paper)**: %s", methods$replicates), 
    "","") %>%
  writeLines()
  ...
  methods_missing <- methods %in% c("unknown", NA)

  new_question(
    sprintf("(section `traits`) Do the methods data data for the trait appear %s correct? %s", trait, 
            ifelse(any(methods_missing), 
                   sprintf("Can you provide missing details for **%s**?",names(methods)[methods_missing] %>% paste0( collapse= ", ")), 
                   "")
            )
    )
  }
...
We have records on the following species from your study. We have attempted to align species names with known taxonomic units, focussing primarily on the [`The Plant List` (TPL)](http://www.theplantlist.org/) -- a global working list of all known plant species. The table below shows the aligned taxnomy, including
