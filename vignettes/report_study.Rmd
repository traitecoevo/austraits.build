...
for(trait in traits_numeric) {
  ...
  c("", "",
    ...
    sprintf("- **Allowable range**: %s - %s %s", definitions$traits$values[[trait]]$values$minimum,
            definitions$traits$values[[trait]]$values$maximum, definitions$traits$values[[trait]]$units),
    sprintf("- **Observed range in this study**: %s - %s %s", data_trait_study %>% dplyr::pull(value) %>% min(),
            data_trait_study %>% dplyr::pull(value) %>% max(), definitions$traits$values[[trait]]$units),
    "", "",
  ...
  # check for outliers
  if(nrow(outliers) > 0 ) {  
    writeLines(sprintf("OUTLIERS: %s of points in your dataset lie in either the bottom or top 2.5 percent of data", nrow(outliers)))
    percent_outliers <- nrow(outliers)/nrow(data_trait_study)*100
    if(percent_outliers > 5) {  
      new_question("(section `traits`) More than 5% of your data points for this trait are outliers, does this seem reasonable, given what you know about the biology of these species and overall distribution of values in Austraits?")
    }
  }
