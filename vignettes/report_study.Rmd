...
for(trait in traits_numeric) {
  ...
  if(nrow(outliers) > 0 ) {  
    ...
    if(percent_outliers > 5) {  
      ...
      new_question(
    sprintf("(section `traits`) Do the data for the trait appear %s correct?", trait)
    ) 
  }
  ...
  if("value_type" %in% names_missing) {
    ...
    sprintf("The variable `value_type` is *'%s'*  Possible values are as follows:", definitions$value_types$description) %>% 
    writeLines() 
