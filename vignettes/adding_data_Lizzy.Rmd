...
1. Columns must include `taxon name`, `site name` (if appropriate), `contexts` (if appropriate), and `sample date` (if appropriate). Trait data can be in either a wide format (1 column for each trait, with `trait name` as the column header) or long format (1 column for all `trait values` and additional columns for `trait name` and `units`) See sections [adding sites](#adding_sites) and [adding contexts](#adding_contexts) below for more information on compiling site and context data.
  ...
  a. If the data were collected at a field site, but there is no site column, add one with a descriptive name:
  ...
  * When there is just a single row of values to summarise, use: 
    ...
    * When you want to choose a different function for each variable, use:
    ...
    ```{r, eval=FALSE, echo=TRUE} 
        read_csv("data/dataset_id/raw/raw_data.csv") %>%
        group_by(individual,`species name`,site,context,etc) %>%
        summarise(mean_buds = mean(buds_per_branch), max_height = max(`plant height`), fire_response = first(`killed or alive`)) %>%
        ungroup()
    ```
...
### Unexpected hangups  
* When Excel saves an `.xls` file as a `.csv` file it only preverses the number of significant figures that are displayed on the screen. This means that if, for some reason, a column has been set to show values a very low number of significant figures or a column is very narrow, data quality is lost.   
* If you're reading a file into R where there are lots of blanks at the beginning of a column of numeric data, the defaults for `read_csv` fail to register the column as numeric. It is fixed by:  
    ```{r, eval=FALSE, echo=TRUE}
...
#### Dataset
  ...
  - *description:* 1-2 sentence description of the studies goals. The abstract of a manuscript usually includes some good sentences/phrases to borrow from.  
...
3.	Add substitutions manually to the metadata.yml file or via the function:
    ...
    For a study requiring a large number of manual taxonomic changes, you can create a dataframe with headers `dataset_id`, `find` (original name), `replace` (APC/APNI accepted name), and `reason` and run:
    
    Begin by generating a list of taxa that haven't been matched:  
    ```{r, eval = FALSE, echo=TRUE}
    austraits$taxa %>% filter (source %in% c("APC", "APNI")) -> recognised_taxa
    austraits$traits %>% filter(dataset_id == "dataset_id") %>%
      distinct(taxon_name) %>%
    ...
    filter(!is.na(replace)) %>%   #to filter out any taxa you didn't match to a new name
    select(dataset_id = dataset_id, find = find, replace = replace, reason = reason) %>% 
    pmap_lgl(metadata_add_taxonomic_change)
    ```
XXXX Daniel - code only reads in 1st line of table as a substitution
...
Identifying duplicates in numeric traits between studies can be difficult, but essential - we attempt to filter out all duplicate occurences of the same measurement. Some common patterns of duplication include:
