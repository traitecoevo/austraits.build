...
## Function to detect if the current operation has been called within remake framework
...
Austraits is built from many different sources, below called "studies" (so called because the sources are most-often individual scientific papers). Our data processing pipeline seeks to combine all of the different studies in a transparent and reproducible way.

Data from each study in Austraits is organised as follows:

- each study is defined by a key comprising the first author and year of publication, e.g. `Falster_2003`, called the `dataset_id`
