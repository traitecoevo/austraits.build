...
Austraits is an open-source compilation of trait data for Australian plant species.


# This study

The austraits compilation includes a dataset under the key **"`r dataset_id`"**. This report describes the information we have on that dataset.
...
Please double check the information under the headings People, Citation, Description to ensure it is correct. Edit the appropriate sections in the metadata.yml file you have been sent if any corrections are required. In particular have we correctly identified your data as having been collected in the field, from a lab experiment, or from herbarium samples.

In addition, if, in the report below, we are missing data of sample sites, sampling strategy, or methods could you please add them to the yml file or send them to us in an alternative format. In particular,
 Site data would hopefully include latitude and longitude coordinates and, where available, variables such as site soil data or climatic data. This information is not required for herbarium studies.
 Sampling strategy is a field to indicate how sites were chosen and how individuals sampled were chosen. If your data is collected from herbarium specimens, you could simply indicate which herbaria were sampled.
 For each trait, we hope to know the value type (is the data point you submitted an individual observation, mean, min, max, etc), number of replicates that contributed to the data point submitted and the methodology used to collect the data. Currently, methods have been extracted from referenced manuscripts, but quite often the submitted dataset includes traits whose collection methods aren't described in the manuscript. For herbarium studies, please specify which variables were measured on dried herbarium samples and which are derived from field observations. For all studies if the value for certain traits (such as plant height) was extracted from the literature, please fill in "published literature" as the method. If a trait value was determined by expert opinion, such as the data collector made the decision that a plant was a shrub, the method would be "expert opinion".

Additionally, as information was added to the metadata.yml file, the following questions specific to your data set arose: **"`r data_study$metadata$dataset$questions_for_author`"**
...
if(nrow(context) > 0 ) {
  ...
  writeLines("**NO CONTEXTUAL SITE DATA AVAILABLE -- can you supply any?**")
}
...
for(trait in traits_numeric) {
  ...
  x <- c("", "",
    ...
    sprintf("- **Standardised units**: %s", definitions$traits$values[[trait]]$units ),
    ...
    "",
    "Your data for this trait have been collected using the following methods:",
    ##TODO - add in data for methods, value type, and replicates from the specific metadata file##
    ##data_study$metadata$traits[[trait]] ##
    "The following plot shows distribution of recorded values for each dataset, with your study name highlighted along the y axis.",
    "")
