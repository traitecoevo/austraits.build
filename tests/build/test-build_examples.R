...
testthat::test_that("test datasets", {
  ...
  Ex3 <- test_build_study(file.path(examples.dir, "test2-metadata.yml"), file.path(examples.dir, "test2-data.csv"), "Example 3")
  
  expect_equal(Ex3$traits$collection_type %>% unique, "wild")
  expect_equal(Ex3$traits %>% filter(collection_type == "wild") %>% nrow(), 406)
  ...
  expect_equal(read_yaml(file.path(examples.dir, "test2-metadata.yml"))$dataset$collection_type, "field")
  expect_equal(read_yaml(file.path(examples.dir, "test2-metadata.yml"))$dataset$sample_age_class, "adult")
  
  
  # Example 4 - Test variables stored as a column in data.csv with missing values are replaced with study level value,
  ...
  # Similar to EX 3 but this time the empty values from the column should be populated with the dataset level value
  Ex4 <- test_build_study(file.path(examples.dir, "test2-metadata.yml"), file.path(examples.dir, "test3-data.csv"), "Example 4")
  
  expect_equal(Ex4$traits$collection_type %>% unique, c("field", "lab", "wild"))
  expect_equal(Ex4$traits %>% filter(collection_type == "field") %>% nrow(), 352)
  expect_equal(Ex4$traits %>% filter(collection_type == "lab") %>% nrow(), 44)
  expect_equal(Ex4$traits %>% filter(collection_type == "wild") %>% nrow(), 10)
  
  expect_equal(Ex4$traits %>% select(trait_name, collection_type) %>% 
                 filter(trait_name == "leaf_N_per_dry_mass") %>%
                 pull(collection_type) %>% unique, c("lab", "wild"))
  expect_equal(Ex4$traits %>% select(trait_name, collection_type) %>% 
                 filter(trait_name == "leaf_N_per_dry_mass") %>%
                 pull(collection_type) %>% grep(pattern = "lab") %>% length , 44)
  expect_equal(Ex4$traits %>% select(trait_name, collection_type) %>% 
                 filter(trait_name == "leaf_N_per_dry_mass") %>%
  ...
  expect_equal(Ex4$traits %>% select(trait_name, collection_type) %>% 
                 ...
                 pull(collection_type) %>% unique, c("field", "wild"))
  expect_equal(Ex4$traits %>% select(trait_name, collection_type) %>% 
                 filter(trait_name == "seed_mass") %>%
                 pull(collection_type) %>% grep(pattern = "field") %>% length , 28)
  expect_equal(Ex4$traits %>% select(trait_name, collection_type) %>% 
                 filter(trait_name == "seed_mass") %>%
                 pull(collection_type) %>% grep(pattern = "wild") %>% length , 1)
  ...
  Ex5 <- test_build_study(file.path(examples.dir, "test3-metadata.yml"), file.path(examples.dir, "test3-data.csv"), "Example 4")
  
  expect_equal(Ex5$traits$collection_type %>% unique, c("field", "leaf_N", "Cape_Tribulation", "wild"))
  expect_equal(Ex5$traits %>% filter(collection_type == "field") %>% nrow(), 81)
  expect_equal(Ex5$traits %>% filter(collection_type == "Cape_Tribulation") %>% nrow(), 271)
  expect_equal(Ex5$traits %>% filter(collection_type == "leaf_N") %>% nrow(), 44)
  expect_equal(Ex5$traits %>% filter(collection_type == "wild") %>% nrow(), 10)
  
  expect_equal(Ex5$traits %>% select(taxon_name, collection_type) %>% 
                 filter(taxon_name == "Trema aspera") %>% pull(collection_type) %>% unique, "wild")
  expect_equal(Ex5$traits %>% select(taxon_name, collection_type) %>% 
                 filter(taxon_name == "Trema aspera") %>% pull(collection_type) %>% length, 10)
  
  expect_equal(Ex5$traits %>% select(trait_name, collection_type) %>% 
                 filter(trait_name == "leaf_N_per_dry_mass") %>%
                 pull(collection_type) %>% grep(pattern ="leaf_N") %>% length, 44)
  expect_equal(Ex5$traits %>% select(trait_name, collection_type) %>% 
                 filter(trait_name == "leaf_N_per_dry_mass") %>%
                 pull(collection_type) %>% grep(pattern ="wild") %>% length, 1)
  
  expect_equal(Ex5$traits %>% select(site_name, collection_type) %>% filter(site_name == "Atherton") %>%
                 pull(collection_type) %>% unique, c("field", "leaf_N"))
  expect_equal(Ex5$traits %>% select(site_name, collection_type) %>% filter(site_name == "Atherton") %>%
                 pull(collection_type) %>% length, 91)
  expect_equal(Ex5$traits %>% select(site_name, collection_type) %>% filter(site_name == "Atherton") %>%
                 pull(collection_type) %>% grep(pattern = "field") %>% length, 81)
  expect_equal(Ex5$traits %>% select(site_name, collection_type) %>% filter(site_name == "Atherton") %>%
                 pull(collection_type) %>% grep(pattern = "leaf_N") %>% length, 10)
  
  expect_equal(Ex5$traits %>% select(site_name, collection_type) %>% filter(site_name == "Cape Tribulation") %>%
                 pull(collection_type) %>% unique, c("Cape_Tribulation", "leaf_N", "wild"))
  expect_equal(Ex5$traits %>% select(site_name, collection_type) %>% filter(site_name == "Cape Tribulation") %>%
                 pull(collection_type) %>% length, 315)
  expect_equal(Ex5$traits %>% select(site_name, collection_type) %>% filter(site_name == "Cape Tribulation") %>%
                 pull(collection_type) %>% grep(pattern = "Cape_Tribulation") %>% length, 271)
  expect_equal(Ex5$traits %>% select(site_name, collection_type) %>% filter(site_name == "Cape Tribulation") %>%
                 pull(collection_type) %>% grep(pattern = "leaf_N") %>% length, 34)
  expect_equal(Ex5$traits %>% select(site_name, collection_type) %>% filter(site_name == "Cape Tribulation") %>%
                 pull(collection_type) %>% grep(pattern = "wild") %>% length, 10)
})
