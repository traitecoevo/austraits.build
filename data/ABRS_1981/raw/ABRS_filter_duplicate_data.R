...
retain_3 <- retain_2 %>% anti_join(retain_trustworthy_traits) %>% anti_join(retain_seed_traits)
...
#    bind_rows(ABRS_old %>% filter(trait_name %in% c("seed_height", "seed_width")) %>% filter(taxon_name == "Acacia gonocarpa")) %>%
#  bind_rows(ABRS_old %>% filter(trait_name %in% c("leaf_length", "leaf_width")) %>% filter(taxon_name %in% c("Nelumbo nucifera", "Ficus carpentariensis"))) %>%
