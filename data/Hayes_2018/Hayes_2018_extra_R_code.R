...
read_csv("data/Hayes_2018/Hayes_et_al._2018_leaf.data.csv") %>%
  ...
  rename(sample = `sample code`) -> leaf_data

read_csv("data/Hayes_2018/Hayes_et_al._2018_cell.data.csv") %>%
  select(-`GPS coords`,-`spectrum.label`) %>%
  ...
  group_by(site.id, cell.type,species, sample, biological.rep) %>%
  summarise_all(.funs = mean) %>%
  ungroup() -> before_spread
...
before_spread %>%
  ...
  rename(epidermis_P = epidermis, hypodermis_P = hypodermis, palisade_mesophyll_P = palisade_mesophyll,
         ...
         sclerenchyma_P = sclerenchyma) -> cell_P_data
...
before_spread %>%
  ...
  select(-species, - site.id, -biological.rep, - `latitude (deg)`, -`longitude (deg)`) %>%
  left_join(cell_P_data, by = "sample") -> cell_data
