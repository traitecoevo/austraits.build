...
data_raw.Herb <- read_csv("data/Ooi_2018/raw/Herbivory.csv", col_types = cols()) 
...
data_raw.height <- read_csv("data/Ooi_2018/raw/Other_data.csv", col_types = cols()) %>% select(species_name = `species`, individual, height)  
data.Herb %>% 
  full_join(by = c("species_name", "individual"),
            data_raw.height
  ) %>% mutate(site_name = "Royal National Park", `latitude (deg)` = -34.114, `longitude (deg)` = 151.066417) %>%
  write_csv("data/Ooi_2018/data.csv", na = "") 
