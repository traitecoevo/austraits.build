...
#pull in data
Water_ForR<-readr::read_csv("C:/Users/chees/OneDrive - James Cook University/Plant_Ecophys_Lab/Buckton/Managed Data/Cheesman_reanalysis/Water_ForR.csv")
Met<-readr::read_csv("E:/Tough_Research/OZ-Work/DRO/DRO_Raymond/Daily_sum.csv")

Met$Date<-as.Date(Met$day,format="%Y-%m-%d")
Rain<-readr::read_csv("C:/Users/chees/OneDrive - James Cook University/Plant_Ecophys_Lab/Buckton/Managed Data/Cheesman_reanalysis/BOM_031012.csv")
Rain$Date<-as.Date(Rain$Date,format="%Y-%m-%d")

Water_ForR$Date<-as.Date(Water_ForR$Date,format="%d/%m/%Y")

summary.water<- ddply(Water_ForR, .(Week,Source), summarise, 
                      Date=min(Date),
                      d18O=mean(d18Oav,na.rm=TRUE),
                      d18O.sd=sd(d18Oav,na.rm=TRUE))
summary.water$Source<-as.factor(summary.water$Source)

summary.water$Source<-factor(summary.water$Source, levels=c("SSM", "DSM", "RW","CW","GW"))


GW<-subset(summary.water,Source=='GW')
mean(GW$d18O)+sd(GW$d18O)
mean(GW$d18O)-sd(GW$d18O)

#GW found only between -4.6703 and -5.3668


CW<-subset(summary.water,Source=='CW')
mean(CW$d18O)+sd(CW$d18O)
mean(CW$d18O)-sd(CW$d18O)

#CW found only between -4.4898 and -5.393917

panel1<-ggplot() +
  geom_rect(data = data.frame(xmin = as.Date(c("2014-03-28")),
                              xmax = as.Date(c("2014-06-30")),
                              ymin = -4.49,
                              ymax = -5.39),
            aes(xmin = xmin, xmax = xmax, ymin = ymin, ymax = ymax),
            fill = "grey80",colour="black", alpha = 0.5) +
    geom_line(data = subset(summary.water,Source=='SSM'|Source=='DSM'),
             aes(x=Date,y=d18O,Group=Source),size=1)+
  geom_errorbar(data = subset(summary.water,Source=='SSM'|Source=='DSM'),
                 aes(x=Date,ymin=(d18O-d18O.sd),ymax=(d18O+d18O.sd)))+
  geom_point(data = subset(summary.water,Source=='SSM'|Source=='DSM'),
             aes(x=Date,y=d18O,fill = Source),shape=21,size=4)+
  scale_fill_grey(start = 0.3, end = 0.8)+
  scale_y_continuous(limits=c(-10,-1.25),breaks=c(-10,-7.5,-5,-2.5))+
...
#isolating RW data and doing some plotting transfromations
RW<-subset(summary.water,Source=='RW')
RW$plot<-RW$d18O*20+200  

#panel 2
panel2<-ggplot() +
  geom_bar(data=Rain,aes(x=Date,y=Rainfall_mm),stat="identity",colour="black",fill="white")+
  scale_x_date(limits=c(as.Date("2014-03-28"),as.Date("2014-06-30")))+
  geom_line(data =RW,aes(x=Date,y=plot))+
  geom_point(data =RW,aes(x=Date,y=plot),shape=22,fill=grey(0.2),size=4)+
    scale_y_continuous(sec.axis = sec_axis(~(.-200)/20,name = expression(paste(delta^18,"O ", " (???)",sep=""))))+
  ylab(expression(paste("Daily Rainfall (mm)",sep="")))+
  theme_bw()+
  theme(
  aspect.ratio=0.5,
  axis.text=element_text(size=20,colour="black"),
...
#multiplot using cowplot   
plot_grid(panel1,panel2,
          align='vh', 
          ncol=1)
...
###Figure 2################
##########################
summary.water$drop1<-(summary.water$Week=="C" & summary.water$Source=="RW")
summary.water$drop2<-(summary.water$Week=="C" & summary.water$Source=="SSM")

water<-subset(summary.water,drop1!='TRUE'&drop2!='TRUE')
max(water)

ggplot() +
  geom_boxplot(data = water,aes(x=Source,y=d18O),
...
#Analysis for Figure2
model<-aov(data=water,d18O~Source)
summary(model)
plot(TukeyHSD(model))
...
Stem_water<-readr::read_csv("C:/Users/chees/OneDrive - James Cook University/Plant_Ecophys_Lab/Buckton/Managed Data/Cheesman_reanalysis/Stem_water_18O.csv")
head(Stem_water)
Stem_water$Date<-as.Date(Stem_water$Date,format="%d/%m/%Y")


summary.stem<- ddply(Stem_water, .(Species), summarise, 
                      #Date=min(Date),
                      Growth.Form=unique(Growth.Form),
                      d18O.av=mean(d18O,na.rm=TRUE),
                      d18O.sd=sd(d18O,na.rm=TRUE))

#Reorder the levels to plot in the correct fashion
summary.stem$Species<-factor(summary.stem$Species, levels=c("TET STIG", "ICH", "MER","NEO","TET","CAL","ENT",
                                                            "SYZ_S","AGY","SYZ_G","ROC","CLE","CAR","MYR","CAS","NOR","END","ALS"))

#Figure3- panel 1
Fig3.a<-ggplot() +
  geom_bar(data = summary.stem,aes(x=Species,y=d18O.av,fill=Growth.Form),
           colour="black",stat="identity",width=0.75)+
  scale_fill_manual(values=c("grey55","lightgrey"))+
  geom_errorbar(data = summary.stem,
                aes(x=Species,ymin=(d18O.av-d18O.sd),ymax=d18O.av),
                width=0.4)+
  theme_bw()+
...
#Figure 3 panel 2
Fig3.b<-ggplot() +
  geom_boxplot(data = subset(summary.water,Source=='SSM'|Source=='DSM'),aes(x=Source,y=d18O,
              fill=Source)) +
  scale_fill_grey(start = 0.3, end = 0.8)+
  theme_bw()+
  scale_y_continuous(limits=c(-6,0))+
...
plot_grid(Fig3.a,Fig3.b, labels = "", align = 'h', rel_widths = c(1,0.15))


#Statisitical tests for stem water

#LDMC nested design species within Growth form
d18O_Model<-aov(d18O~Growth.Form+Growth.Form:Species,data=Stem_water)
summary(d18O_Model)
LDMC.F.value=0.1262/1.9989
LDMC.F.value
LDMC.pvalue=1-pf(LDMC.F.value,1,16)
LDMC.pvalue

##################################
#Suplimnetary figure 1
...
Sup.1a<-
  ggplot()+
    stat_smooth(data=Stem_water,aes(Date,y=d18O),
              method='lm',size=1,colour="black",linetype=2,
              se=FALSE)+
  geom_point(data=Stem_water,aes(x=Date,y=d18O,fill=Growth.Form),
               colour="black",shape=22,size=4)+
    scale_fill_manual(values=c("grey55","lightgrey"))+
  
  theme_bw()+
  ylab(expression(paste(delta^18,"O"[Xylem], " (???)",sep="")))+
  scale_x_date(limits=c(as.Date("2014-03-28"),as.Date("2014-06-30")))+
  theme(
    #aspect.ratio=0.5,
    axis.text=element_text(size=20,colour="black"),
...
Sup.1b<-ggplot()+
  ...
  scale_fill_manual(values=c("grey55","lightgrey"))+
  stat_smooth(method='lm',data = subset(summary.water,Source=='SSM'|Source=='DSM'),
             aes(x=Date,y=d18O),size=1,colour="black",linetype=2,se=FALSE)+
  geom_errorbar(data = subset(summary.water,Source=='SSM'|Source=='DSM'),
               aes(x=Date,ymin=(d18O-d18O.sd),ymax=(d18O+d18O.sd)))+
  theme_bw()+
  ylab(expression(paste(delta^18,"O"[Soil], " (???)",sep="")))+
  scale_x_date(limits=c(as.Date("2014-03-28"),as.Date("2014-06-30")))+
  theme(
    #aspect.ratio=0.5,
    axis.text=element_text(size=20,colour="black"),
...
plot_grid(Sup.1a, Sup.1b, 
          align='vh', 
          ncol=1)

#Export as 550 x550

#stats of supplimentary figure 1
  
model1<-lm(data = subset(summary.water,Source=='SSM'|Source=='DSM'),d18O~yday(Date))
summary(model1)

model2<-lm(data = Stem_water,d18O~yday(Date))
summary(model2)
...
ggplot() +
  geom_point(data= subset(summary.water,Source=='SSM'|Source=='DSM'),aes(x=Date,y=d18O,fill=Source))+
  scale_x_date(limits=c(as.Date("2014-03-28"),as.Date("2014-06-30")))+
scale_x_date()
...
ggplot()+
  ...
  #          aes(xmin = xmin, xmax = xmax, ymin = ymin, ymax = ymax),
  #          fill = "grey", alpha = 0.5)+
  geom_point(data=summary.water,aes(x=Date,y= d18O,shape=Source))
  
  
  aes(shape=Source))+
    geom_rect(aes(xmin='2014-04-01',xmax='2014-04-04',ymin=-4.57,ymax=-5.730))
    scale_fill_manual(values=c("grey55","lightgrey"))+
  theme_bw()+
...
#Figure 5
##
Manus<-readr::read_csv("C:/Users/chees/OneDrive - James Cook University/Plant_Ecophys_Lab/Buckton/Managed Data/Cheesman_reanalysis/Manuscript_Table.csv")
