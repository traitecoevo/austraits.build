read_csv("data/eFLOWER_2020/raw/data_raw.csv") %>% 
  rename(trait_name = Character) %>%
  distinct(Character_state,.keep_all = TRUE) %>% write_csv("data/eFLOWER_2020/raw/traits_values1.csv")
...
character_values %>%
  ...
  ungroup() %>%
  write_csv("data/eFLOWER_2020/data.csv")

#excluded data:
subset(austraits$excluded_data,dataset_id==current_study & error!="Missing value") %>%
  write_csv("data/eFLOWER_2020/raw/excluded_data.csv")

subset(austraits$traits,dataset_id==current_study) %>%
  write_csv("data/eFLOWER_2020/raw/data_in_Austraits.csv")
