...
metadata_study <- read_metadata(sprintf("data/%s/metadata.yml", dataset_id))
...
if(!is.null(metadata_study$questions) & !is.na(metadata_study$questions)) {

  x <- names(metadata_study$questions)[ !(names(metadata_study$questions) %in% c(
  ...
  for(i in seq_along(x))
    new_question(sprintf("(section `general`) %s", metadata_study$questions[[i]]))
}
```
...
metadata_study$people %>%
  list_to_df() %>%
  dplyr::select(role, name, institution) %>%
  my_kable_styling()
```
...
```{r, results='asis', echo=FALSE}
n <- length(metadata_study$source)
if(n > 1) {
  writeLines("Secondary citations:")

  for(i in 2:n) {
    metadata_study$source[[i]] %>%
    list1_to_df() %>%
    my_kable_styling() %>% 
    cat()
    writeLines("\n\n")
...
"  "
  ...
  if(nrow(subset(data_study$excluded_data,error == "Observation excluded in metadata")) > 0) { 
    metadata_study$exclude_observations %>%
      list_to_df() %>%
      my_kable_styling() %>%
      writeLines()
  }
...
define_variable <- function(var, definitions) {

  description <- switch(var,
                        value_type = definitions$value_type$description,
                        replicates = definitions$austraits$elements$traits$elements$replicates,
                        methods = definitions$austraits$elements$methods$elements$methods
                        )
...
for(trait in traits_numeric) {
  ...
  transforms <- metadata_study$traits %>% list_to_df() %>% filter(trait_name == trait)
  ...
  writeLines(ifelse(nrow(subset(data_study$excluded_data, error == "Value out of allowable range" & trait_name == trait)) > 0,
...
if(nrow(subset(data_study$excluded_data, error == "Value out of allowable range" & trait_name == trait)) > 0 ) {
...
for(trait in traits_categorical) {
  ...
  transforms <- metadata_study$traits %>% list_to_df() %>% filter(trait_name == trait)
  substitutions_for_trait <- metadata_study$substitutions %>% list_to_df() %>% filter(trait_name == trait)
...
```{r, results='asis', echo=FALSE}
definitions$austraits$elements$taxa$elements %>%
  list1_to_df() %>%
  my_kable_styling()
```
...
data_study$taxa %>%
  select(one_of(names(definitions$austraits$elements$taxa$elements))) -> tmp
...
data_study$taxonomic_updates %>% select(-dataset_id) %>% 
  ...
  mutate(
    d1 = map2_dbl(original_name, cleaned_name, ~adist(.x, .y)) %>% as.character() %>% str_replace("0", ""),
    d2 = map2_dbl(taxon_name, cleaned_name, ~adist(.x, .y)) %>% as.character() %>% str_replace("0", ""),
...
data_study$taxa %>%
  ...
  select(one_of(names(definitions$austraits$elements$taxa$elements))) %>%
