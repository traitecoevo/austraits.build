...
site_main <- c("site_name", "longitude (deg)","latitude (deg)", "description")
sites <-
 ...
 select(location_id, site_name, records, `latitude (deg)`, `longitude (deg)`, description)
...
if(nrow(filter(data_study$excluded_data, error == "Value out of allowable range" & trait_name == trait)) > 0 ) {
  data_study$excluded_data %>% filter(error == "Value out of allowable range" & trait_name == trait) %>%
    select(observation_id, taxon_name, site_name, value, unit) %>%
    my_kable_styling() %>%
    writeLines()
}
