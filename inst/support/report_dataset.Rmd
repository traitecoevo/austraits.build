...
site_main <- c("location_name", "longitude (deg)","latitude (deg)", "description")
sites <-
...
if( any(tolower(sites$location_name) %in% c(NA, "unknown") ) ) {
...
if(length(context_properties) > 0 ) {
  context_properties %>%
    for (i in 1:length(context_properties)) {      
      writeLines("The column titled `r context_properties[[i]]$category[1]` in your dataset was mapped to contextual property `r context_properties[[i]]$context_property[1]`, within the category `r context_properties[[i]]$category[1]`")

      writeLines("The following values exist for this context: ")

      context_properties[[i]] %>% select(find, replace, description)  %>%
        my_kable_styling() %>%
        writeLines()
    }
...
if(nrow(filter(data_study$excluded_data, error == "Value out of allowable range" & trait_name == trait)) > 0 ) {
  data_study$excluded_data %>% filter(error == "Value out of allowable range" & trait_name == trait) %>%
    select(observation_id, taxon_name, location_name, value, unit) %>%
    my_kable_styling() %>%
    writeLines()
}
