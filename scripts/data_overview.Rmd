...
There are __`r nrow(austraits$traits)`__ trait records contained within Austraits database which belong to __`r austraits$traits$taxon_name %>% unique() %>% length()`__ species and __`r austraits$taxa$family %>% unique() %>% length()`__ plant families.
...
for(trait in traits_numeric) {
  ...
  c("", "",
    ...
    sprintf("- **records - number spp**: %s",   n_distinct(data_trait$taxon_name)),
