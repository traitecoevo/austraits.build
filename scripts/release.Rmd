...
---
title: Instructions for creating a new data release
...
create_release <- function(austraits, v_prev= NULL) {
  ...
  filename <- sprintf("%s/austraits-%s-flattened.rds", export_dir, version_number)

  austraits_flattened |>
    saveRDS(filename)

  filename <- sprintf("%s/austraits-%s-flattened.parquet", export_dir, version_number)
...
2. Update `master`. On `master` branch,
  ...
  - use `merge commit` NOT `squash and merge` for the merge into master
...
create_release(austraits, v_prev = "6.0.0")
```
 - the function above creates a set of files for release, including `austraits-X.X.X.rds`, `austraits-X.X.X-flattened.rds`, `austraits-X.X.X-flattened.parquet`, `austraits-X.X.X.zip`, `dictionary.html`, `news.md`
...
- change bits in pages
  - version number
  - date
- in intro page, version number for files
