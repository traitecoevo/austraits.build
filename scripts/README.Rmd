...
- ` `r sprintf("austraits-%s.zip", desc::desc_get_version())` `: data in plain text format (.csv, .bib, .yml files). Suitable for anyone, including those using Python. 
- ` `r sprintf("austraits-%s.rds", desc::desc_get_version())` `: data as compressed R object. Suitable for users of R (see below).
...
A schema and definitions for the database are given in the files/components `schema` and `traits`, available within the download. The file `dictionary.html` provides the same information in textual format. Full details on each of these components and columns are contained within the definition. Similar information is available at [http://traitecoevo.github.io/austraits.build/articles/definitions.html](http://traitecoevo.github.io/austraits.build/articles/definitions.html) and [http://traitecoevo.github.io/austraits.build/articles/austraits_database_structure.html](http://traitecoevo.github.io/austraits.build/articles/austraits_database_structure.html).
