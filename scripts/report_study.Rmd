...
1. Filling in the [response survey](https://forms.gle/vhZgxDd734dDddG88) with answers to the questions we ask throughout the document, and summarised at the end of the document.
2. Sending edited versions of the input files for your study: ` `r dataset_id`/data.csv` and ` `r dataset_id`/metadata.yml` (if appropriate) to email [austraits.database@gmail.com](mailto:austraits.database@gmail.com).
...
if(nrow(sites_extra) > 0 ) {
  ...
  writeLines("No site data available.")
}
...
if(nrow(context) > 0 ) {
  ...
  writeLines("No contextual data available.")
}
...
We were unable to find taxonomic matches for the following species in the data. Where genus and family are shown, the specified genus aligns with an accepted genus.
