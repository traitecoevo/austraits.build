...
We have records on `r data_study$traits$species_name %>% unique() %>% length()` species from your study. We have attempted to align species names with [`The Plant List` (TPL)](http://www.theplantlist.org/) -- a global working list of all known plant species. The full list of names for species recorded in your study are listed below.

# Trait measurements

## Overview

The dataset includes `r data_study$traits %>% nrow()` individual data points from `r data_study$traits$species_name %>% unique() %>% length()` species, with data included for the following `r data_study$traits$trait_name %>% unique() %>% length()` traits:
...
```{r corrections, results='hide', echo=FALSE}

corrections <- tibble(original_name= character(), notes = character())

if(suppressWarnings(!is.null(metadata_study$taxonomic_updates) & !is.na(metadata_study$taxonomic_updates))) {
  corrections <- metadata_study$taxonomic_updates %>%
    list_to_df() %>%
    select(original_name = find, notes = reason)
}

species <- data_study$traits %>%
  select(original_name, species_name) %>%
  distinct() %>%
  arrange(original_name) %>%
  left_join(austraits$taxonomy, by ="species_name") %>%
...
We have records on the following species from your study. You'll note that we have attempted to align species names with known taxonomic units, focussing primarily on the [`The Plant List` (TPL)](http://www.theplantlist.org/) -- a global working list of all known plant species. In addition we have tried to align these names with the [`Australian Plant Census` (APC)](https://biodiversity.org.au/nsl/services/apc) and the [`Australian Plant Names Index` (APNI)](https://biodiversity.org.au/nsl/services/APNI). Our main reason for focussing on TPL is the existence of automated tools for querying the database. In the future, we will look to align primarily with APC.  The table below shows the aligned taxonomy, with columns:

```{r, results='asis', echo=FALSE}
definitions$austraits$elements$taxonomy$elements %>%
  list1_to_df() %>%
  bind_rows(
    tibble(
        key=c("original_name", "notes", "genus"),   
        value=c("Name provided by contrtibutor", "The reason a name change was implemented (where relevant)", "Genus of the aligned species"))) %>%
  mutate(key = gsub("species_name", "TPL_name", key, fixed=TRUE)) %>%
  arrange(match(key, names(species))) %>%
  my_kable_styling()
```

Updated names are shown only where different to the original name supplied. We also include links to the species records in the relevant systems. Clicking on the links will take you to the relevant taxonomic record in each system for that species.
